...
ggplot(fourstates, aes(x = year, y = total_yield, color = state)) +
  ...
  theme_bw() +
  scale_y_log10(name = 'total yield (bu)') +
...
Let's say we want to fit a linear mixed model to the total yield data, to determine if there is a trend over time in total yield. In SAS we could use `proc reg`.
...
```{r}
yield_fit <- lm(total_yield ~ year, data = se_soybeans)
```
...
In both the R and SAS output, the residual diagnostic plots show that the assumptions of normally distributed residuals with homogeneous error variance are severely violated. One way to deal with this is to log-transform the response variable before fitting the model. When we do that, we are looking at multiplicative changes in yield over time, rather than additive. 

In SAS, to the best of my knowledge, you have to pre-transform the response variable before fitting the model, for most statistical procedures.
...
In contrast, in R we can include the log-transformation directly inside the model statement. This is a good practice because some functions that we may use later for post-hoc comparisons and hypothesis tests can detect that a transformation was done and automatically do back-transformations where needed.

```{r}
logyield_fit <- lm(log(total_yield) ~ year, data = se_soybeans)

summary(logyield_fit)
anova(logyield_fit)
plot(logyield_fit)
```

The residual diagnostic plots show great improvement.

### Mixed models

The models we fit above are not really the most appropriate. That's because linear regression assumes that all data points are independent of one another. Clearly that is not the case for the time series of yield for each state, because yield values from the same state in successive years are not independent of one another. Our dataset has a "multilevel" or "nested" structure. In particular, there are repeated measures across time within each individual state.   

FIXME PUT SOME MORE WORDS IN HERE
...
### A note on Bayesian generalized mixed models

So far, I've presented "classical" or frequentist statistical models. But if you know me, you know I am a big advocate for Bayesian statistical analysis. I am not going to go into it in this lesson, but I wanted to make sure you are aware that state-of-the-art algorithms for fitting Bayesian mixed models are available in both SAS and R. 

If you are a die-hard SAS user and want to fit Bayesian models, I would recommend looking into **proc bglimm**, which stands for **B**ayesian **g**eneralized **li**near **m**ixed **m**odels. In R, I recommend the **brms** package, which stands for **B**ayesian **r**egression **m**odel**s**. Both those packages use the Hamiltonian Monte Carlo algorithm to fit Bayesian mixed models. Check out the [**brms** lesson](https://quentinread.com/SEAStats/#bayesianmixedmodels) on the SEAStats page if you want a quick introduction to that package.
