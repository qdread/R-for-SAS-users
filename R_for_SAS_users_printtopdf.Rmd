...
Go to the following page for hints if you get stuck. Answers follow the hints.
...
- *Exercise 2 hint*: Use `filter()` with two conditions, one for `year` and one for `gen`, separated by `,`.
- *Exercise 3 hint*: Use `group_by()` and `summarize()`, with `mean()` for the mean and `sd()` for the standard deviation.
- *Exercise 4 hint*: Use `aes(x = loc, y = yield)`.
- *Exercise 5 hint*: You can still use `lm()` to fit the model, with `gen` as the predictor variable.
- *Exercise 6 hint*: The right-hand side of the model formula will now be `gen + loc + gen:loc`.
